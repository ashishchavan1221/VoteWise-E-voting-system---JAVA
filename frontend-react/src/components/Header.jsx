        ...
        return (
            <nav className="bg-[#0f172a] text-white py-4 px-6 border-b border-white/5 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <Link to="/" className="text-3xl font-bold">Vote<span className="text-blue-500">Wise</span></Link>
                    <div className="flex items-center gap-6">
                        <Link to="/results" className="font-semibold text-gray-300 hover:text-white transition group flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-3 bg-gray-800 px-4 py-2 rounded-full border border-gray-700 shadow-inner">
                            <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center font-bold shadow-[0_0_10px_rgba(37,99,235,0.8)]">
                                {currentUser ? currentUser.charAt(0) : 'U'}
                            </div>
                            <span className="font-semibold">{currentUser || 'Guest'}</span>
                        </div>
                        <button onClick={() => { localStorage.removeItem('currentUser'); window.location.href = '/'; }} className="w-10 h-10 bg-red-500/10 text-red-500 rounded-full flex items-center justify-center hover:bg-red-500 hover:text-white transition shadow-lg">
    ...
    return (
        <nav className="sticky top-0 left-0 right-0 z-50 bg-[#0f172a] border-b border-white/5 shadow-xl">
            <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
                <Link to="/" className="flex items-center gap-3 bg-[#1e293b] border border-white/10 px-5 py-2.5 rounded-full shadow-lg hover:-translate-y-1 transition duration-300 group hover:shadow-blue-500/20">
                    <div className="w-10 h-10 bg-gradient-to-tr from-blue-600 to-indigo-500 rounded-full flex items-center justify-center overflow-hidden shadow-[0_0_15px_rgba(59,130,246,0.5)] transform group-hover:-rotate-12 transition-transform duration-300">
                        <img src="/src/assets/images/images.png" className="w-full h-full object-cover p-1 rounded-full mix-blend-screen" />
                    </div>
                    <span className="text-2xl font-extrabold tracking-tight text-white pr-2 transition-all">Vote<span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Wise</span></span>
                </Link>
                ...
                <div className="hidden md:flex items-center gap-1 bg-[#1e293b] border border-white/10 p-1.5 rounded-full shadow-inner">
                    ...
                    <Link to="/results" className="px-5 py-2 text-sm font-semibold text-gray-300 hover:text-white hover:bg-white/10 rounded-full transition-all flex items-center gap-2">
